...
-- Filter out unmatching records and the records where the FlightDate does not fall into the time period
Flight1_data = FILTER Flight1 BY (Origin eq 'ORD' AND Dest neq 'JFK') AND (Cancelled neq 1.0 AND Diverted neq 1.0)
	       AND (ToDate(FlightDate,'yyyy-MM-dd') <= ToDate('2008-05-31','yyyy-MM-dd'))
	       AND (ToDate(FlightDate,'yyyy-MM-dd') >= ToDate('2007-06-01','yyyy-MM-dd'));
